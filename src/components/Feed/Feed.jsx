...
import { BiCommentAdd, BiDotsVertical, BiDotsVerticalRounded, BiDownArrow, BiHeart, BiLeftArrow, BiSolidHeart, BiSolidLeftArrow, BiSolidRightArrow, BiTrash } from "react-icons/bi";
                    ...
                    {feedPosts.length > 0 ? feedPosts.map((post, index) => {
                        ...
                        return (
                            <div key={index} className="post bg-gray-100 mb-4 rounded-lg px-4 py-2 w-full drop-shadow-lg">
                                ...
                                <div className="middle-section flex flex-row w-80 break-all px-0 py-4">
                                    {post.post.content}
                                </div>
                                ...
                                <form action="" className="flex flex-row justify-between mb-4 drop-shadow-lg items-center">
                                    <input required className="input w-10/12 px-4 py-2 rounded-full flex flex-row items-center" type="text" placeholder="Make a Comment" onChange={handleCommentChange} />
                                ...
                                <ul className="flex flex-col absolute w-1/2 top-20 right-full bg-white px-4 py-2 rounded-lg drop-shadow-sm" style={{ display: openPostLike ? "flex" : "none" }}>
                                    <p className="text-xs text-gray-400 py-2 ">Post Liked by</p>
                                    <div className="absolute top-3/7" style={{ right: "-10px" }}><BiSolidRightArrow color="white" /></div>
                                ...
                                <ul className="">
                                    {post.post.comments && post.post.comments.length > 0 ? post.post.comments.map((el, i) => {
                                        ...
                                        return (
                                            <div className="flex flex-col mb-4 drop-shadow-lg px-2 py-4 border border-solid border-gray-200 rounded-lg" key={i}>
                                                <div className="w-full flex flex-row justify-between">
                                                    ...
                                                    <div className="flex flex-col justify-center items-center">
                                                        <div className="flex flex-row justify-center items-center">
                                                            <div className="flex flex-row" onClick={handleCLike}>
                                                        ...
                                                        <ul style={{ display: openCLike ? "flex" : "none" }} className="flex flex-col w-3/4 absolute left-full bg-white px-4 py-2 rounded-lg drop-shadow-sm">
                                                ...
                                                <div className="flex w-80 py-2 break-all" >
                                                    {el.comment}
                                                </div>
