...
const PostCard = ()=>{
    ...
    return(
        <div className="flex flex-col justify-between items-center px-4 py-4">
                <div className="flex flex-row px-5 py-4 justify-evenly items-center border border-gray-200 border-solid rounded-lg w-11/12">
                    <img src="/d-prof.jpg" className="rounded-full" width={200} height={200} alt="Profile" />
                    <div>
                        <p className="text-5xl"><b>{uname}</b></p>
                        <p style={{whiteSpace:"pre-wrap"}} className="text-xs text-color-800">{bio}</p>
                    </div>
                    {username === getCookie('socio-user') && (
                        <button className="" onClick={() => setUpdateMode(!updateMode)}>Update Profile</button>
                    )}
                </div>
                <div>
                    <div  >
